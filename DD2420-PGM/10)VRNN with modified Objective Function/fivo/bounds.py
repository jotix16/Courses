...
def entropy_criterion(num_samples, log_weights_acc, unused_t):
  """A criterion that resamples based on entropy of the weights."""
  return -tf.reduce_sum(log_weights_acc,axis=0) >= tf.reduce_sum(num_samples / 2.0)
...
def fivo(cell,
  ...
  def while_step(t, rnn_state, tas, accs):
    ...
    should_resample = -tf.reduce_sum(log_weights_acc,axis=0) >= tf.reduce_sum(num_samples / 2.0)
    ## GIVEN
  ...
  log_p_hat += (tf.reduce_logsumexp(final_log_weights, axis=0) - tf.log(tf.to_float(num_samples)))
